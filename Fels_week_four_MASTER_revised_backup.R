...
mytable2 <- xtabs(~ year+conlegis2+race, data=gss)
mytable2 <- prop.table(mytable2, 1) # row proportions
...
plot2 <- plot2 + labs(title="Percent of Americans Hardly Any Confidence \nin Congress by Race", x="Year", y="%", 
                      fill = "Race")
...
mydata4$sex <- as.character(mydata4$sex)
